...
function DiscoveryEventCard({ event }) {
  ...
  const { discoveryLatitude, discoveryLongitude } = useSelector((store) => store.map);
  ...
  const getCategoryIcon = (name) => {
    ...
    return (
      images[path]?.default || images['../../../../assets/categoryImages/gif/default.gif'].default
    );
  };
          ...
          <div className="flex-row">
            <div>
              <IconButton disabled>
                ...
                >
                  {!event.appUser?.profilePhotoId && event.appUser.firstName?.[0].toUpperCase()}
                </Avatar>
              </IconButton>
            </div>
            <div>
          ...
          <div className="flex-column-justify-end" style={{ marginRight: '10px' }}>
            {event.eventCategories.map((c) => (
              <div key={c.category.categoryId}>
                <img
                  src={getCategoryIcon(c.category.categoryName)}
                  alt={c.category.categoryName}
                  width={85}
                  height={85}
                />
              </div>
        ...
        <div className="flex-row-justify-start">
          <div className="discovery-e-c-register">
            {!event.registered ? (
              ...
              <Button variant="outlined" color="success" sx={{ textTransform: 'none' }} disabled>
                {tButton('registered')}
              </Button>
            )}
          </div>
          <div className="discovery-e-c-register">
            <Button
              variant={
                event.latitude == discoveryLatitude && event.longitude == discoveryLongitude
                  ? 'contained'
                  : 'outlined'
              }
              sx={{ textTransform: 'none' }}
