...
export const DeleteEventRegister = createAsyncThunk('deleteMyRegistiretion', async (data) => {
  await axios.delete(`EventRegisters/DeleteEventRegister/${data.userId}/${data.eventId}`);
});
...
export const GetMyCurrentEvents = createAsyncThunk('myCurrentEvents', async (id) => {
...
export const GetMyFutureEvents = createAsyncThunk('myFutureEvents', async (id) => {
...
export const eventRegisterSlice = createSlice({
  ...
  reducers: {
    ...
    MarkEventIsRated: (state, action) => {
      ...
      const registiration = state.myRegisteredEvents.find((e) => e.eventRegisterId == eRegisterId);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(RegisterEvent.fulfilled, (state, action) => {
        if (!action.payload) {
          state.eventRegisterResponse = 'eventRegister.registerEventFulfilledError';
          state.eventRegisterMistakeAlert = true;
        }
      })
      .addCase(RegisterEvent.rejected, (state) => {
        state.eventRegisterResponse = 'rejected';
        state.eventRegisterMistakeAlert = true;
      })
      ...
      .addCase(RateEvent.fulfilled, (state, action) => {
        if (!action.payload) {
          state.eventRegisterResponse = 'eventRegister.rateEventFulfilledError';
